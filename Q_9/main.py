...
G.sort(key=operator.attrgetter('maintbudget'))
B.sort(key=operator.attrgetter('typ'))
...
for i in range(0,100):
	if not G or not B:
		break
	boyalternatives=B[:k]
	girlalternatives=G[:k]
	boy=random.choice(boyalternatives)
	girl=random.choice(girlalternatives)
	...
	if boy.readytopair(girl) and girl.readytopair(boy) and boy.currStatus()=='S' and girl.currStatus()=='S':
		...
		s1=boy.name+' of type : '+boy.typ+' is gonna date '+girl.name+' of maintenamce budget: '+str(girl.maintbudget)
		commit.append(s1)
		#print(s1)
		B.remove(boy)
		G.remove(girl)
...
giftBasket.sort(key=operator.attrgetter('value'),reverse=True)

for c in couplesList:

	giftBasket2=giftBasket[:]
	before=c.budget
	spent=0
	while before<=0 or spent>=c.maintbudget or giftBasket2:
		giftalternatives=giftBasket2[:k]
		if giftalternatives:
			item=random.choice(giftalternatives)
